...
class TestDense:
    ...
    def test_can_create_dense_linop(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
        dense = dense_cls(self.ref)
        assert dense == dense

    def test_can_create_dense_linop_with_dim(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
    ...
    def test_can_copy_construct(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
    ...
    def test_can_create_dense_from_1D_np_array(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
    ...
    def test_can_read_mtx_file(self, data_type: pg.types.ValueType):
    ...
    def test_dense_support_basic_functionality(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
        dense = dense_cls(
            self.ref, (3, 3), np.array([self.values], dtype=data_type.numpy_type), 3
        )
    ...
    def test_can_apply_to_transpose(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
    ...
    def test_can_transpose(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
        dense_a = dense_cls(
            self.ref, (3, 3), np.array([self.values], dtype=data_type.numpy_type), 3
        )
    ...
    def test_add_scaled(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
    ...
    def test_sub_scaled(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
        a = np.array(self.values, dtype=data_type.numpy_type)
        dense_a = dense_cls(a)
        alpha = 0.5
    ...
    def test_dense_can_return_size(self, data_type: pg.types.ValueType):
        dense_cls = getattr(pGB.matrix, "dense_" + data_type)
        dense = dense_cls(
            self.ref, (3, 3), np.array(self.values, dtype=data_type.numpy_type), 3
        )
