...
#define GKO_MATRIX_BINDING(Name)                                               \
  py::class_<gko::matrix::Name<ValueType, IndexType>,                          \
             std::shared_ptr<gko::matrix::Name<ValueType, IndexType>>,         \
             gko::LinOp>(module_matrix, "Name", py::buffer_protocol())         \
      .def(py::init([](std::shared_ptr<gko::Executor> exec) {                  \
        return gko::matrix::Name<ValueType, IndexType>::create(exec);          \
      }))                                                                      \
      ...
      .def(py::init([](std::shared_ptr<gko::Executor> exec, py::tuple dim,     \
        ...
        return gko::share(gko::matrix::Name<ValueType, IndexType>::create(     \
