...
def asarray(obj, executor="Reference", dtype="float"):
    valid_dtypes = ["int", "float", "double"]
    if not dtype in valid_dtypes:
        raise ValueError("Not a valid dtype: " + dtype + " possible choices are: " + valid_dtypes) 
    valid_executor = ["Reference", "Cuda"]
    if not executor in valid_executor:
        raise ValueError("Not a valid executor: " + dtype + " possible choices are: " + valid_executor) 
