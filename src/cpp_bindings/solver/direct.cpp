...
void init_direct(py::module_ &module_solver)
{
    py::class_<gko::experimental::solver::Direct<ValueType,IndexType>,
               std::shared_ptr<gko::experimental::solver::Direct<ValueType,IndexType>>, gko::LinOp>(
        module_solver, "direct")
        .def(py::init([](std::shared_ptr<gko::Executor> exec,
                ...
                if (factorization == "Cholesky"){
                    factorization_fact = gko::share(gko::experimental::factorization::Cholesky<ValueType, IndexType>::build().with_skip_sorting(true).on(exec));
                } 
                if (factorization == "LU"){
                    factorization_fact = gko::share(gko::experimental::factorization::Lu<ValueType, IndexType>::build().with_skip_sorting(true).on(exec));
                }
        ...
        .def("__repr__",
             [](const gko::experimental::solver::Direct<ValueType,IndexType> &o) {
